...
class LinuxHardwareInfo:
    ...
    def hardware_collector(self):
        self.utils.head("硬件信息收集")
        print("")
        print("请稍候，我们正在收集您的硬件详情")
        print("")
        self.result = {}

        steps = [
            ('正在收集 PCI 设备', self.pci_devices, None),
            ('正在收集主板信息', self.motherboard, "Motherboard"),
            ('正在收集 BIOS 信息', self.bios, "BIOS"),
            ('正在收集 CPU 信息', self.cpu, "CPU"),
            ('正在收集 GPU 信息', self.gpu, "GPU"),
            ('正在收集显示器信息', self.monitor, "Monitor"),
            ('正在收集网络信息', self.network, "Network"),
            ('正在收集音频信息', self.sound, "Sound"),
            ('正在收集 USB 控制器', self.usb_controllers, "USB Controllers"),
            ('正在收集输入设备', self.input, "Input"),
            ('正在收集存储控制器', self.storage_controllers, "Storage Controllers"),
            ('正在收集生物识别信息', self.biometric, "Biometric"),
            ('正在收集蓝牙信息', self.bluetooth, "Bluetooth"),
            ('正在收集 SD 控制器信息', self.sd_controller, "SD Controller"),
            ('正在收集系统设备', self.system_devices, "System Devices")
        ]
        ...
        for index, (message, function, attribute) in enumerate(steps, start=1):
            ...
            if value:
                ...
                print("    - 未找到 {}。".format(attribute.lower()))
